...
#read in leprosy association statistics at HLA
...
mali.ld <- read.table("hla_primary.mali.ld", header = T)
mali.ld$SNP_B <- as.character(mali.ld$SNP_B)
total$rsid <- as.character(total$rsid)
total$mali.ld <- mali.ld$R2[match(total$rsid, mali.ld$SNP_B)]
...
total$bin_r2.2 <- 1
total$bin_r2.2[which(total$mali.ld>0.1 & total$mali.ld <= 0.3)] <- 2
total$bin_r2.2[which(total$mali.ld>0.3 & total$mali.ld <= 0.5)] <- 3
total$bin_r2.2[which(total$mali.ld>0.5 & total$mali.ld <= 0.7)] <- 4
total$bin_r2.2[which(total$mali.ld>0.7)] <- 5

total$bin_dp <- 1
total$bin_dp[which(total$dp>0.6 & total$dp <= 0.7)] <- 2
total$bin_dp[which(total$dp>0.7 & total$dp <= 0.8)] <- 3
total$bin_dp[which(total$dp>0.8 & total$dp <= 0.9)] <- 4
total$bin_dp[which(total$dp>0.9)] <- 5
...
total$annotate <- 0
total$annotate[c(which(total$rsid=="HLADQB1_402"), which(total$rsid=="rs9270296:T"), which(total$rsid=="HLAB_4901"))] <- 1

cols3 <- brewer.pal(8,"Paired")

#plot primary HLA association
primary_hla <- ggplot(total, aes(x=position, y=-log10(pval))) + 
  ...
  geom_point(data=subset(total, hla==1 & pval<0.0007), color=cols3[2], size=4, shape=5, stroke = 1.2) + 
  ylab("-log P-value") + 
  xlab(NULL) + 
...
p1 <- (primary_hla/genes/recomb_rate) + plot_layout(heights = c(3, 1, 1))
