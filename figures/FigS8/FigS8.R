...
#read in X Y channel intensities for Mali samples
mali_m <- read.table("mali.means.txt", header = T)
#define outliers
l5 <- aberrant(mali_m, lambda = 5)

#highlight outliers
p_mali_m <- mali_m
p_mali_m$out <- c(0)
p_mali_m$out[l5$outlier] <- c(1)

#plot X/Y channel intensities highlighting outliers
p.mali = ggplot(p_mali_m, aes(X, Y)) + 
  geom_point(aes(colour = factor(out)), size = 4) + scale_colour_manual(name = "exclude", values =c(cols[8], cols[2])) +
  theme_bw() +
  ylab("mean Y") +
  xlab("mean X") +
  ggtitle("Mali") +
...
#read in X Y channel intensities for Malawi samples
malawi_m <- read.table("malawi.means.txt", header = T)
#identfy outliers
l5 <- aberrant(malawi_m, lambda = 5)

#highlight outliers
p_malawi_m <- malawi_m
p_malawi_m$out <- c(0)
p_malawi_m$out[l5$outlier] <- c(1)

#plot X/Y channel intensities highlighting outliers
p.malawi = ggplot(p_malawi_m, aes(X, Y)) + 
  geom_point(aes(colour = factor(out)), size = 4) + scale_colour_manual(name = "exclude", values =c(cols[8], cols[2])) +
  theme_bw() +
  ylab("mean Y") +
  xlab("mean X") +
