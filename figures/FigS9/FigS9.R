...
#aberrant function taken from: https://www.well.ox.ac.uk/~spencer/Aberrant/
aberrant <-
...
#read in heterozygosity/missingness data for Malawi
hm = read.table("malawi.het_miss.txt", header = TRUE)
hm$het = (hm$nm-hm$o_hom)/hm$nm
hm$logit_miss = log(hm$miss/(1-hm$miss))

#identify outliers
l5 <- aberrant(cbind(hm$het, hm$logit_miss), lambda = 20)

#highlight outliers
hm$out <- c(0)
hm$out[l5$outlier] <- c(1)

#plot heterozysity/missingness highlighting outlier samples - Malawi
p.malawi = ggplot(hm, aes(logit_miss, het)) + 
  geom_point(aes(colour = factor(out)), size = 4) + 
  scale_colour_manual(name = "exclude", values =c(cols[8], cols[2])) +
  theme_bw() +
  ylab("Heterozygosity") +
  xlab("logit(Missingness)") +
  ggtitle("Malawi") +
...
#read in heterozygosity/missingness data for Malawi
hm = read.table("mali.het_miss.txt", header = TRUE)
#identify outliers
l5 <- aberrant(cbind(hm$het, hm$logit_miss), lambda = 20)
...
#plot heterozysity/missingness highlighting outlier samples - Mali
p.mali = ggplot(hm, aes(logit_miss, het)) + 
  geom_point(aes(colour = factor(out)), size = 4) + 
  scale_colour_manual(name = "exclude", values =c(cols[8], cols[2])) +
  theme_bw() +
  ylab("Heterozygosity") +
